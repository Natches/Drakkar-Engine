...
void ThreadPool::addTask(Task* pTask, VArgs&&...args) {
	static U16 last = 0;
	last >= static_cast<U16>(m_pool.size()) ? last = 0 : last = last;
	m_pool[last]->addTask(pTask);
...
template<class ...VArgs>
U16 ThreadPool::addTask(U16 threadNumber, Task* pTask, VArgs&&...args) {
	if (threadNumber == static_cast<U16>(m_pool.size()))
		threadNumber = 0;
	m_pool[threadNumber]->addTask(pTask);
	if constexpr (static_cast<bool>(sizeof...(VArgs)))
